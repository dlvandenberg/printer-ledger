...
func NewPrint(p Print, s Settings, ledgers SpoolLedgers) (Print, error) {
...
func EditedPrint(stored PrintLedger, p Print, ledgers SpoolLedgers) (Print, error) {
	released := ledgers.released(stored.Print)
...
func validatePrint(p Print, ledgers SpoolLedgers, v *ValidationError) {
	if p.Quantity < 1 {
...
// left.
func validateUsages(p Print, ledgers SpoolLedgers, v *ValidationError) {
	printType := p.FilamentType(ledgers.Spools())
	requested := map[int64]unit.Grams{}
	overdrawn := map[int64]bool{}
	for row, usage := range p.Usages {
		ledger, ok := ledgers.find(usage.SpoolID)
		switch {
		case !ok:
...
func (p Print) WithRates(s Settings, ledgers SpoolLedgers) Print {
	usages := make([]FilamentUsage, 0, len(p.Usages))
	for _, usage := range p.Usages {
		if ledger, ok := ledgers.find(usage.SpoolID); ok {
	...
	p.KwhPerHour = s.PowerRate(p.FilamentType(ledgers.Spools())).KwhPerHour
...
func (p Print) WithFrozenRatesOf(stored Print, ledgers SpoolLedgers) Print {
...
func (p Print) FilamentType(spools Spools) FilamentType {
	for _, usage := range p.Usages {
		if spool, ok := spools.find(usage.SpoolID); ok {
...
func repriced(usages []FilamentUsage, stored Print, ledgers SpoolLedgers) []FilamentUsage {
	...
	for _, usage := range usages {
		...
		} else if ledger, found := ledgers.find(usage.SpoolID); found {
			usage.CostPerGram = ledger.Spool.GramPrice()
		}
...
func (p Print) SpoolColors(spools Spools) []string {
