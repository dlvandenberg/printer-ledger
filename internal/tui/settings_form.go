...
func newSettingsForm(s app.SettingsView) *form {
	specs := []fieldSpec{
		textSpec{Key: domain.FieldKwhPrice, Label: "Electricity per kWh", Placeholder: "0.28", Prefill: unit.FormatCents(s.KwhPrice)},
	}
	for _, rate := range s.PowerRates {
		specs = append(specs, textSpec{
	...
	specs = append(specs,
		textSpec{Key: domain.FieldMachineHourlyRate, Label: "Machine per hour", Placeholder: "0.35", Prefill: unit.FormatCents(s.MachineHourlyRate)},
		textSpec{Key: domain.FieldPrinterPurchaseCost, Label: "Printer purchase cost", Placeholder: "399.00", Prefill: unit.FormatCents(s.PrinterPurchaseCost)},
		textSpec{Key: domain.FieldDefaultMargin, Label: "Default margin", Placeholder: "50%", Prefill: unit.FormatPercent(s.DefaultMargin)},
		textSpec{Key: domain.FieldMinMargin, Label: "Minimum margin", Placeholder: "15%", Prefill: unit.FormatPercent(s.MinMargin)},
	)
